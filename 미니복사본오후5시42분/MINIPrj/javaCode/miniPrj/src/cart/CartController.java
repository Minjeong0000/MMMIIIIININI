...
public class CartController {
	...
	//----------------------------------------민정:장바구니비우기-- CartController클래스 ----
	public void emptyCartMenu() throws Exception {//public이어야 다른 클래스에서 쓸 수 있더라구요,,
		System.out.println("장바구니를 비우시겠습니까? (Y/N)");
		String answer = JDBCTemplate.SC.nextLine();
	
		switch(answer) {
		
		case "Y" :emptyCart(); break;
		case "N" :System.out.println("이전 메뉴로 돌아갑니다");return; 
	...
	private void emptyCart() throws Exception {
		...
		try {
			if(Main.selectBeverage != null) {
				...
				conn.commit(); // 트랜잭션 커밋
				System.out.println("음료 장바구니에 담긴 상품이 없습니다");
		...
		try {
			if(Main.selectFood != null) {
				...
				conn.commit(); // 트랜잭션 커밋
				System.out.println("푸드 장바구니에 담긴 상품이 없습니다");
		...
		try {
			if(Main.selectMerchandise != null) {
				...
				conn.commit(); // 트랜잭션 커밋
				System.out.println("MD 장바구니에 담긴 상품이 없습니다");
